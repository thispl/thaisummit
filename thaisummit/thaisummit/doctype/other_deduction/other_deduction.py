...
class OtherDeduction(Document):
	@frappe.whitelist()
	def get_payroll_date(self):
		date = datetime.strptime(self.deduction_date, '%Y-%m-%d').date()
		if date.day < 31:
	...
	def on_submit(self):
		if self.workflow_state == 'Approved':
			adds = frappe.db.exists('Additional Salary',{'employee':self.employee,'payroll_date':self.payroll_date,'docstatus':('!=',2),'salary_deduction':'Other Deduction'})
			if adds:
				...
				doc.salary_component = "Other Deduction"
				doc.amount = self.amount
				doc.save(ignore_permissions=True)
				doc.submit()
				frappe.db.commit()
				frappe.db.set_value('Other Deduction',self.name,'additional_salary',doc.name)
