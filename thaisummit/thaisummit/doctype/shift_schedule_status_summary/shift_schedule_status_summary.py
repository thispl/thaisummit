...
def get_shift_status(doc):
	...
	for d in depts:
		date1 = frappe.db.sql("select name,docstatus from `tabShift Assignment` where department = '%s' and '%s' between start_date and end_date and docstatus != '2' order by creation desc"%(d.name,dates[0]),as_dict=True)
		date2 = frappe.db.sql("select name,docstatus from `tabShift Assignment` where department = '%s' and '%s' between start_date and end_date and docstatus != '2' order by creation desc"%(d.name,dates[1]),as_dict=True)
		date3 = frappe.db.sql("select name,docstatus from `tabShift Assignment` where department = '%s' and '%s' between start_date and end_date and docstatus != '2' order by creation desc"%(d.name,dates[2]),as_dict=True)
		date4 = frappe.db.sql("select name,docstatus from `tabShift Assignment` where department = '%s' and '%s' between start_date and end_date and docstatus != '2' order by creation desc"%(d.name,dates[3]),as_dict=True)
		date5 = frappe.db.sql("select name,docstatus from `tabShift Assignment` where department = '%s' and '%s' between start_date and end_date and docstatus != '2' order by creation desc"%(d.name,dates[4]),as_dict=True)
		date6 = frappe.db.sql("select name,docstatus from `tabShift Assignment` where department = '%s' and '%s' between start_date and end_date and docstatus != '2' order by creation desc"%(d.name,dates[5]),as_dict=True)
		frappe.errprint(date1)
		frappe.errprint(date2)
		frappe.errprint(date3)
		frappe.errprint(date4)
		frappe.errprint(date5)
		if date1:
			if date1[0].docstatus == 1:
				mark1 = '<td style="background-color:#7dcea0">✓</td>'
			elif date1[0].docstatus == 0:
				mark1 = '<td style="background-color:#f0b27a">-</td>'
		else:
			mark1 = '<td style="background-color:#f95848">X</td>'
		if date2:
			if date2[0].docstatus == 1:
				mark2 = '<td style="background-color:#7dcea0">✓</td>'
			elif date2[0].docstatus == 0:
				mark2 = '<td style="background-color:#f0b27a">-</td>'
		else:
			mark2 = '<td style="background-color:#f95848">X</td>'
		if date3:
			if date3[0].docstatus == 1:
				mark3 = '<td style="background-color:#7dcea0">✓</td>'
			elif date3[0].docstatus == 0:
				mark3 = '<td style="background-color:#f0b27a">-</td>'
		else:
			mark3 = '<td style="background-color:#f95848">X</td>'
		if date4:
			if date4[0].docstatus == 1:
				mark4 = '<td style="background-color:#7dcea0">✓</td>'
			elif date4[0].docstatus == 0:
				mark4 = '<td style="background-color:#f0b27a">-</td>'
		else:
			mark4 = '<td style="background-color:#f95848">X</td>'
		if date5:
			if date5[0].docstatus == 1:
				mark5 = '<td style="background-color:#7dcea0">✓</td>'
			elif date5[0].docstatus == 0:
				mark5 = '<td style="background-color:#f0b27a">-</td>'
		else:
			mark5 = '<td style="background-color:#f95848">X</td>'
		if date6:
			if date6[0].docstatus == 1:
				mark6 = '<td style="background-color:#7dcea0">✓</td>'
			elif date6[0].docstatus == 0:
