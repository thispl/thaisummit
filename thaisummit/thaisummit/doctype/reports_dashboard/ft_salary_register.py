...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    ws.merge_cells(start_row=1, start_column=24, end_row=1, end_column= 29)
...
def title1(args):
    ...
    data = ["TSAI FT  Wages for the Month of Sep-2021",'','','','','','','','','Standard Structure Month','','','',
...
def get_column(args):
    ...
    "Gross","PF","ESI","Can","P Tax","PPE","Total","Net Wage","Bonus"]
...
def get_data(args):
    ...
    dedcution_comp = ["Provident Fund","Employee State Insurance","Canteen Charges","Professional Tax","Personal Protective Equipment",]

    # if args.department:
    #     salary_slips = frappe.get_all("Salary Slip",{'employee_type':'BC''start_date':args.from_date,'end_date':args.to_date},['*'])	

    if args.employee:
        salary_slips = frappe.get_all("Salary Slip",{'employee_type':'FT','employee':args.employee,'start_date':args.from_date,'end_date':args.to_date},['*'])	
    
    else:
        salary_slips = frappe.get_all("Salary Slip",{'employee_type':'FT','start_date':args.from_date,'end_date':args.to_date},['*'])	
    i =1
