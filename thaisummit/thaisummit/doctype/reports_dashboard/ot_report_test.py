...
def get_data(args):
        ...
        for dept in departments:
            ...
            for date in dates:
                ...
                if args.employee_type:
                    ...
                    `tabEmployee` on `tabOvertime Request`.employee = `tabEmployee`.name  where `tabEmployee`.employee_type = '%s' and `tabEmployee`.department = '%s' and `tabOvertime Request`.ot_date = '%s' and `tabOvertime Request`.workflow_state in ('Pending for HOD','Approved') and `tabOvertime Request`.docstatus != '2' """%(args.employee_type,dept.name,date),as_dict=1)
                else:
                    ots = frappe.db.sql("""select `tabOvertime Request`.employee as employee,`tabOvertime Request`.ot_hours as ot_hours from `tabOvertime Request` left JOIN
                    `tabEmployee` on `tabOvertime Request`.employee = `tabEmployee`.name  where `tabEmployee`.department = '%s' and `tabOvertime Request`.ot_date = '%s' and `tabOvertime Request`.workflow_state in ('Pending for HOD','Approved') and `tabOvertime Request`.docstatus != '2' """%(dept.name,date),as_dict=1)
                # ots = frappe.get_all("Overtime Request",{"department":dept.name,"ot_date":date,"workflow_state":('in',("Pending for HOD","Approved")),'docstatus':('!=',2)},["employee","ot_hours"])
                for ot in ots:
                    ot_hrs = ot.ot_hours
                    if ot_hrs:
    ...
    total_mfg_ot_amt = 0
    mfg_dept_list = tuple(mfg_dept_list)
    for date in dates:
        mfg_t_ot = 0
        mfg_ot_hrs = timedelta(0,0,0)
        if args.employee_type:
            mfg_ot = frappe.db.sql("""select `tabOvertime Request`.employee as employee,`tabOvertime Request`.ot_hours as ot_hours from `tabOvertime Request` left JOIN
            `tabEmployee` on `tabOvertime Request`.employee = `tabEmployee`.name  where `tabEmployee`.employee_type = '%s' and `tabEmployee`.department in %s and `tabOvertime Request`.ot_date = '%s' and `tabOvertime Request`.workflow_state in ('Pending for HOD','Approved') and `tabOvertime Request`.docstatus != '2' """%(args.employee_type,format(mfg_dept_list),date),as_dict=1)
        else:
            mfg_ot = frappe.db.sql("""select `tabOvertime Request`.employee as employee,`tabOvertime Request`.ot_hours as ot_hours from `tabOvertime Request` left JOIN
            `tabEmployee` on `tabOvertime Request`.employee = `tabEmployee`.name  where `tabEmployee`.department in %s and `tabOvertime Request`.ot_date = %s and `tabOvertime Request`.workflow_state in ('Pending for HOD','Approved') and `tabOvertime Request`.docstatus != '2' """%(format(mfg_dept_list),date),as_dict=1)
        # mfg_ot = frappe.get_all("Overtime Request",{"department":('in',(mfg_dept_list)),"ot_date":date,"workflow_state":('in',("Pending for HOD","Approved")),'docstatus':('!=',2)},["employee","ot_hours"])
        for mf_ot in mfg_ot:
            ...
            if mfg_ot_hrs:
                ...
                ftr = [3600,60,1]
                hr = (sum([a*b for a,b in zip(ftr, map(int,str(str(hours) +':'+str(minutes)+':00').split(':')))]))/3600
                amt_per_hr = ((frappe.db.get_value("Employee",mf_ot.employee,'basic')/26)/8)*2
                mfg_ot_amt = hr * amt_per_hr
                mfg_t_ot += round(mfg_ot_amt)
                total_mfg_ot_amt += round(mfg_ot_amt)
...
def get_tsai(args):
    data = []
    dates = get_dates(args)
    tsai_ot_amount_header = ['','OT Amount']
    ...
    for date in dates:
        ...
        if args.employee_type:
            ...
            mfg_ot = frappe.db.sql("""select `tabOvertime Request`.employee as employee,`tabOvertime Request`.ot_hours as ot_hours from `tabOvertime Request` left JOIN
            `tabEmployee` on `tabOvertime Request`.employee = `tabEmployee`.name  where `tabEmployee`.department in %s and `tabOvertime Request`.ot_date = '%s' and `tabOvertime Request`.workflow_state in ('Pending for HOD','Approved') and `tabOvertime Request`.docstatus != '2' """%(format(dept_list),date),as_dict=1)
        # mfg_ot = frappe.get_all("Overtime Request",{"department":('in',(dept_list)),"ot_date":date,"workflow_state":('in',("Pending for HOD","Approved")),'docstatus':('!=',2)},["employee","ot_hours"])
        for ot in mfg_ot:
            ot_hrs = ot.ot_hours
            day = ot_hrs.days * 24
            hours = day + ot_hrs.seconds // 3600
