...
@frappe.whitelist()
def download():
    filename = 'N-2 Sequence Plan vs Stock Report'
...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    ws['I1'].fill = PatternFill(fgColor="52c132", fill_type = "solid")
    ...
    ws.cell(row=1,column=14).alignment = Alignment(horizontal='center')
    ...
    for rows in ws.iter_rows(min_row=1, max_row=len(master)+2, min_col=1, max_col=18):
        for cell in rows:
            cell.border = border
...
def add_header_title():
    header = ["N-2 SEQUENCE PLAN VS STOCK REPORT",'','','',"DELIVERY PLAN",'','','','STOCK','','','','',format_date(today()),'','','UPCOMING DAYS','']
    return header
...
def add_header(dates):
    ...
    d5 = d5.strftime('%d/%m')
    header = ["MAT NO",'PART NO','PART NAME','DAILY ORDER',d1,d2,d3,'TOTAL PLAN','FG','WELD','HPS','TOTAL STOCK','COVERAGE','SHORT','PROD PLAN','STATUS',d4,d5]
    return header
    ...
    for m in master:
        ...
        for date in dates:
            ...
            if qty != '-':
                days += 1
            qty_list.append(qty)
        row.append(daily_order[0].total/days)
        row.extend(qty_list)
        row.append(daily_order[0].total)
        ...
        if type(hps) == int:
            total_stock += hps
        coverage = total_stock/(daily_order[0].total/days)
        short = daily_order[0].total - total_stock
        if short < 0:
            short = 0
        prod_plan = frappe.db.get_value('SAP Production Plan',{'product_no':m.mat_no,'order_date':date},'planned_quantity') or '-'
        status = ''
        if type(prod_plan) == int:
            if prod_plan <= int(daily_order[0].total):
                status = 'Check'
            else:
                status = 'OK'
        else:
