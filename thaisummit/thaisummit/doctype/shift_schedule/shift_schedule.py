...
@frappe.whitelist()
def add_header(w):
	w.writerow(["ID", "Name", "Department", "Department Code", "Type", "Shift","Route No"])
	return w
...
def get_data(args):
	...
	for employee in employees:
		row = [
			employee.name, employee.employee_name, employee.department,frappe.db.get_value("Department",employee.department,"cost_centre"),employee.employee_type,employee.default_shift,employee.route_no or 'NB'
		]
		data.append(row)
	return data
...
def get_active_employees(args):
	employees = frappe.db.get_all('Employee',
		fields=['name', 'employee_name', 'department', 'date_of_joining', 'company', 'relieving_date','employee_type','default_shift','route_no'],
		filters={
			'docstatus': ['<', 2],
			'status': 'Active',
