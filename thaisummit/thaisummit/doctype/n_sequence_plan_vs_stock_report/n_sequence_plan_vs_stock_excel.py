...
@frappe.whitelist()
def download():
    filename = 'N Sequence Plan vs Stock Report'
...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    ws['F1'].fill = PatternFill(fgColor="52c132", fill_type = "solid")
    ...
    ws.cell(row=1,column=11).alignment = Alignment(horizontal='center')
    ...
    for rows in ws.iter_rows(min_row=1, max_row=len(master)+2, min_col=1, max_col=13):
        for cell in rows:
            cell.border = border
...
def add_header_title():
    header = ["N SEQUENCE PLAN VS STOCK REPORT",'','','',"DELIVERY PLAN",'STOCK','','','','',format_date(today()),'','']
    return header

def add_header(dates):
    d1 = datetime.strptime(dates[0],'%Y-%m-%d')
    d1 = d1.strftime('%d/%m')
    header = ["MAT NO",'PART NO','PART NAME','DAILY ORDER',d1,'FG','WELD','HPS','TOTAL STOCK','COVERAGE','SHORT','PROD PLAN','STATUS']
    return header
