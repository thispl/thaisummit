...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    ws.append(['All CL Manpower Summary ' + args.from_date + ' to '+ args.to_date,'','','','','','','','','','','','','','','','','','','',''])
    ...
    for dept in department:
        ...
        if not basic == 0: 
            ot = others/basic*100
        ws.append([i,'All CL',dept.name,head_count,person_count,basic,others,'',shift,transport,attendance,'','',ppe,'','',total,deduction,canteen,grand_total,ot])
        i = i + 1
    ...
    for header in ws.iter_rows(min_row=len(department)+4, max_row=len(department)+4, min_col=17, max_col=17):
         for cell in header:
             cell.fill = PatternFill(fgColor='ffe135', fill_type = "solid")
    ...
    for rows in ws.iter_rows(min_row=1, max_row=len(department)+3, min_col=1, max_col=21):
        for cell in rows:
            cell.border = border

    ws.append(['','','','','','','','','','','','','','','','','','','','','','','','',''])
    ws.append(['Total','List','Wage','','','OT','','','','','','','','','','','','','',''])
    ws.append(['','','QTY','RATE','AMOUNT','QTY','RATE','AMOUNT','Shift Allowance','Transportation','Attendance','SV Salary','Insurance','PPE','Service Charge','Sourcing Fee','Total','CGST@9%','SGST@9%','Invoice Amount'])
    designation = ['Line Leader','Skilled','Fork Lift','Semi Skilled','Trainee','Un Skilled'] 
    total_head = 0
    total_amount = 0
    total_rate = 0
    total_ot_amount = 0
    ...
    for d in designation:
        basic = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as basic from `tabSalary Slip`
        left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Basic' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation = '%s' """%(dept.name,args.from_date,args.to_date,d),as_dict=True)[0].basic or 0
        head_count = frappe.db.sql(""" select sum(payment_days) as payment_days from `tabSalary Slip` where department = '%s' and start_date = '%s' and end_date = '%s' and employee_type = 'CL' and designation = '%s'"""%(dept.name,args.from_date,args.to_date,d),as_dict=True)[0].payment_days or 0
        shift = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as shift from `tabSalary Slip`
        left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Shift Allowance' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation = '%s' """%(dept.name,args.from_date,args.to_date,d),as_dict=True)[0].shift or 0
        transport = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as transport from `tabSalary Slip`
        left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Transport Allowance' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation = '%s'"""%(dept.name,args.from_date,args.to_date,d),as_dict=True)[0].transport or 0
        attendance = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as attendance from `tabSalary Slip`
        left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Attendance Allowance' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation= '%s' """%(dept.name,args.from_date,args.to_date,d),as_dict=True)[0].attendance or 0
        ppe = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as ppe from `tabSalary Slip`
        left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'PPE Cost' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation= '%s' """%(dept.name,args.from_date,args.to_date,d),as_dict=True)[0].ppe or 0
        others = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as others from `tabSalary Slip`
        left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Others' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation = '%s' """%(dept.name,args.from_date,args.to_date,d),as_dict=True)[0].others or 0
        rate = 0
        if not head_count == 0:
            rate = basic/head_count
        total = total_amount + total_ot_amount + shift + transport + attendance + ppe
        cgst = total*9/100
        sgst = total*9/100
        invoice_amount = cgst + sgst
        ws.append(['Total',d,head_count,rate,basic,'','',others,shift,transport,attendance,'','',ppe,'','',total,cgst,sgst,invoice_amount])
        total_head = total_head + head_count
        total_amount = total_amount + basic
        total_rate = total_rate + rate
        total_ot_amount = total_ot_amount +others
        total_shift = total_shift + shift
        total_transport =total_transport + transport
        total_attendance = total_attendance + attendance
        total_ppe = total_ppe + ppe
    ...
    ws.append(['','Total',total_head,total_rate,total_amount,'','',total_ot_amount,total_shift,total_transport,total_attendance ,'','',total_ppe,'','',total,total_cgst,total_sgst,total_invoice_amount])
    ws.append(['','','','','','','','','','','','','','','','','','',''])
    
    for header in ws.iter_rows(min_row=len(department)+6, max_row=len(department)+13, min_col=1, max_col=1):
         for cell in header:
             cell.fill = PatternFill(fgColor='ffe135', fill_type = "solid")

    for header in ws.iter_rows(min_row=len(department)+14, max_row=len(department)+14, min_col=17, max_col=17):
         for cell in header:
             cell.fill = PatternFill(fgColor='ffe135', fill_type = "solid")

    ws.merge_cells(start_row=len(department)+8, start_column=1, end_row=len(department)+13, end_column=1)
    for rows in ws.iter_rows(min_row = len(department)+8, max_row = len(department)+13, min_col=1, max_col=1):
        for cell in rows:
    ...
    for c in contractor:
        for d in designation:
            basic = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as basic from `tabSalary Slip`
            left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Basic' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation = '%s' and contractor = '%s' """%(dept.name,args.from_date,args.to_date,d,c.name),as_dict=True)[0].basic or 0
            head_count = frappe.db.sql(""" select sum(payment_days) as payment_days from `tabSalary Slip` where department = '%s' and start_date = '%s' and end_date = '%s' and employee_type = 'CL' and designation = '%s' and contractor = '%s' """%(dept.name,args.from_date,args.to_date,d,c.name),as_dict=True)[0].payment_days or 0
            shift = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as shift from `tabSalary Slip`
            left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Shift Allowance' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation = '%s' and contractor = '%s' """%(dept.name,args.from_date,args.to_date,d,c.name),as_dict=True)[0].shift or 0
            transport = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as transport from `tabSalary Slip`
            left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Transport Allowance' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation = '%s' and contractor = '%s' """%(dept.name,args.from_date,args.to_date,d,c.name),as_dict=True)[0].transport or 0
            attendance = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as attendance from `tabSalary Slip`
            left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Attendance Allowance' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation= '%s' and contractor = '%s' """%(dept.name,args.from_date,args.to_date,d,c.name),as_dict=True)[0].attendance or 0
            ppe = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as ppe from `tabSalary Slip`
            left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'PPE Cost' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation= '%s' and contractor = '%s' """%(dept.name,args.from_date,args.to_date,d,c.name),as_dict=True)[0].ppe or 0
            others = frappe.db.sql("""select sum(`tabSalary Detail`.amount) as others from `tabSalary Slip`
            left join `tabSalary Detail` on `tabSalary Slip`.name = `tabSalary Detail`.parent where `tabSalary Slip`.department = '%s' and `tabSalary Detail`.salary_component = 'Others' and `tabSalary Slip`.start_date ='%s' and `tabSalary Slip`.end_date = '%s' and employee_type = 'CL' and designation = '%s' and contractor = '%s' """%(dept.name,args.from_date,args.to_date,d,c.name),as_dict=True)[0].others or 0
            rate = 0
            if not head_count == 0:
                rate = basic/head_count
            total = total_amount + total_ot_amount + shift + transport + attendance + ppe
            ...
            sgst = total*9/100
            invoice_amount = cgst + sgst
            ws.append([c.name,d,head_count,rate,basic,'','',others,shift,transport,attendance,'','',ppe,'','',total,cgst,sgst,invoice_amount])
            total_head = total_head + head_count
            total_amount = total_amount + basic
            total_rate = total_rate + rate
            total_ot_amount = total_ot_amount +others
    ...
    for header in ws.iter_rows(min_row=len(department)+6, max_row=len(department)+7, min_col=1, max_col=21):
         for cell in header:
             cell.fill = PatternFill(fgColor='6B8E23', fill_type = "solid")
    
    
    # ws.merge_cells(start_row=len(department)+6, start_column=3, end_row=len(department)+6, end_column=5)
    ws.merge_cells(start_row=len(department)+6, start_column=3, end_row=len(department)+6, end_column=5)
    ws.merge_cells(start_row=len(department)+6, start_column=6, end_row=len(department)+6, end_column=8) 
    for rows in ws.iter_rows(min_row = len(department)+6, max_row = len(department)+6, min_col=3, max_col=6):
        for cell in rows:
            cell.alignment = Alignment(horizontal='center')

    # for rows in ws.iter_rows(min_row = len(department)+6, max_row = len(department)+6, min_col=7, max_col=9):
    #     for cell in rows:
    #         cell.alignment = Alignment(horizontal='center')
    i= len(department)+16
    for c in contractor:
        ws.merge_cells(start_row=i, start_column=1, end_row=i+5, end_column=1)
        i = i + 8

    # i= len(department)+16
    # for c in contractor:
    #     for rows in ws.iter_rows(min_row = i, max_row = i+5, min_col=1, max_col=1):
    #         for cell in rows:
    #             cell.alignment = Alignment(vertical='center')
    #         i = i + 8
    for cell in ws["A:A"]:
    ...
    for rows in ws.iter_rows(min_row= len(department)+6, max_row=len(department)+6+len(designation)+3+len(department)+2+len(designation)+3*12, min_col=1, max_col=21):
        for cell in rows:
            cell.border = border
