...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    date_object1 = datetime.strptime(str(args.from_date), "%Y-%m-%d")
    date_object2 = datetime.strptime(str(args.to_date), "%Y-%m-%d")
    day1 = date_object1.day
    month1 = date_object1.month
    day2 = date_object2.day
    month2 = date_object2.month

    ws.append([str(contractor) + ' Manpower Summary ' + str(day1) + '/' + str(month1) + ' to '+ str(day2) + '/' + str(month2) ,'','','','','','','','','','','','','','','','','','','',''])
    ws.append(['','','','','','Income','','','','','','','','','','','','Deduction','','Grand Total','% OT'])
    ...
    ws.merge_cells(start_row=2, start_column=21, end_row=3, end_column=21)
    ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=5)
    ...
    for cell in ws["1:1"]:
        cell.alignment = Alignment(horizontal='center')
    for rows in ws.iter_rows(min_row = 2, max_row = 3, min_col=1, max_col=21):
        for cell in rows:
            cell.alignment = Alignment(horizontal='center')
