...
class HRPROCLPlanvsActualReport(Document):
    @frappe.whitelist()
    def shiftwise_report(self):
        ...
        for dg in dept_group:
            ...
            for dept in departments:
                plan=frappe.db.count("Shift Assignment",{'department':dept.name,'start_date':self.date,'shift_type':self.shift,'employee_type':'CL'})
                actual_shift=frappe.db.count("QR Checkin",{'department':dept.name,'shift_date':self.date,'qr_shift':self.shift,'ot':0,'employee_type':'CL'})
                shift_difference=plan-actual_shift
                ot_count=frappe.db.count("QR Checkin",{'department':dept.name,'shift_date':self.date,'qr_shift':self.shift,'ot':1,'employee_type':'CL'})
                if g == 1:
        ...
        total_plan=frappe.db.count("Shift Assignment",{'start_date':self.date,'shift_type':self.shift,'employee_type':'CL'})
        total_actual_shift=frappe.db.count("QR Checkin",{'shift_date':self.date,'qr_shift':self.shift,'ot':0,'employee_type':'CL'})
        total_shift_difference=total_plan-total_actual_shift
        total_ot_count=frappe.db.count("QR Checkin",{'shift_date':self.date,'qr_shift':self.shift,'ot':1,'employee_type':'CL'})
    ...
    def withoutshift_report(self):
        ...
        for dg in dept_group:
            ...
            g = 1
            for dept in departments:
                plan=frappe.db.count("Shift Assignment",{'department':dept.name,'start_date':self.date,'employee_type':'CL'})
                actual_shift=frappe.db.count("QR Checkin",{'department':dept.name,'shift_date':self.date,'ot':0,'employee_type':'CL'})
                shift_difference=plan-actual_shift
                ot_count=frappe.db.count("QR Checkin",{'department':dept.name,'shift_date':self.date,'ot':1,'employee_type':'CL'})
                if g == 1:
                    data += '<tr style="font-size:5px;padding:0px"><td style="border: 1px solid black;padding:0px" rowspan="%s" ><b><center>%s</center></b></td><td style="border: 1px solid black;padding:2px"><b>%s</b></td><td style="border: 1px solid black;padding:0px"><b><center>%s</center></b></td><td style="border: 1px solid black;padding:0px"><b><center>%s</center></b></td><td style="border: 1px solid black;padding:0px"><b><center>%s</center></b></td><td style="border: 1px solid black;padding:0px"><b><center>%s</center></b></td></tr>'%(len(departments),dg,dept.name,plan,actual_shift,shift_difference,ot_count)
                    g = 0
                else:
        ...
        total_plan=frappe.db.count("Shift Assignment",{'start_date':self.date,'employee_type':'CL'})
        total_actual_shift=frappe.db.count("QR Checkin",{'shift_date':self.date,'ot':0,'employee_type':'CL'})
        total_shift_difference=total_plan-total_actual_shift
        total_ot_count=frappe.db.count("QR Checkin",{'shift_date':self.date,'ot':1,'employee_type':'CL'})
        data += '<tr style="background-color:#ff9900;font-size:10px"><td style="border: 1px solid black" colspan="2"><center><b>Total</b><center></td><td style="border: 1px solid black"><b><center>%s</center></b></td><td style="border: 1px solid black"><b><center>%s</center></b></td><td style="border: 1px solid black"><b><center>%s</center></b></td><td style="border: 1px solid black"><b><center>%s</center></b></td></tr>'%(total_plan,total_actual_shift,total_shift_difference,total_ot_count)
        data +='</table>'
        return data