...
def get_data(dates):
    ...
    # forecast_items = frappe.db.sql("select distinct `tabForecast Data`.mat_no as item from `tabForecast Data` where not exists(select `tabTSAI BOM`.item from `tabTSAI BOM` where `tabTSAI BOM`.item = `tabForecast Data`.mat_no)",as_dict=True)
