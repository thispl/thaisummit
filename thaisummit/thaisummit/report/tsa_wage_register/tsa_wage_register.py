...
def get_columns(filters):
    ...
    _("SHT") + ":Currency/:100",_("ARR") + ":Currency/:100",_("Additional Allowance") + ":Currency/:150",_("Welding Allowance") + ":Currency/:150",_("PP Allowance") + ":Currency/:100",_("Transport Allowance") + ":Currency/:100",_("Other Allowance") + ":Currency/:100",_("Others") + ":Currency/:100",_("Gross") + ":Currency/:100",_("PF") + ":Currency/:100",_("ESI") + ":Currency/:100",
...
def get_data(filters):
    ...
    earning_comp = ['Basic','House Rent Allowance','Other Allowance','Position Allowance','Attendance Bonus','Shift Allowance','Arrear','Additional Allowance','Welding Allowance','PPE Allowance','Transport Allowance','Other Allowance',]
    dedcution_comp = ['Provident Fund','Employee State Insurance','Canteen Charges','Professional Tax','LWF','TELEXP','PPE','ADV']

    if filters.department:
        salary_slips = frappe.get_all("Salary Slip",{'employee_type':filters.employee_type,'department':filters.department,'start_date':filters.from_date,'end_date':filters.to_date},['*'])	

    if filters.employee:
        salary_slips = frappe.get_all("Salary Slip",{'employee_type':filters.employee_type,'employee':filters.employee,'start_date':filters.from_date,'end_date':filters.to_date},['*'])	
    
    else:
        salary_slips = frappe.get_all("Salary Slip",{'employee_type':filters.employee_type,'start_date':filters.from_date,'end_date':filters.to_date},['*'])	
