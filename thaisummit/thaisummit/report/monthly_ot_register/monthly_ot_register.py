...
def get_data(filters):
    ...
    for emp in employees:
        ...
        basic_value = frappe.db.get_value("Employee",{'employee':emp.name,'employee_name':emp.employee_name,'department':emp.department,'date_of_joining':emp.date_of_joining},['basic'])
        if emp.employee_type != 'CL':
            ot_amount = (((basic_value/26)/8)*2)*total_ot_hr
        else:
            ot_amount = ((basic_value/8)*2)*total_ot_hr
        row.append(basic_value)
...
def get_employees(filters):
    ...
    employees = frappe.db.sql("""select name, employee_name, department, date_of_joining,employee_type from `tabEmployee` where status = 'Active' %s and vacant = 0 """%(conditions),as_dict=True)
    return employees
