...
@frappe.whitelist()
def get_tag_list():
    total_tbs = frappe.db.sql("""select * from `tabTSAI Part Master` where mat_type in ('INH','BOP') and customer in ('RE')""",as_dict=1)
    updated_tbs_list = []
    updated_tbs_dict = {}
...
def get_live_stock(mat_no):
    ...
    if response:
        ...
        if stocks:
            ica = frappe.db.sql(
                "select warehouse from `tabInventory Control Area` where transfer_plan = 'Y' ", as_dict=True)
