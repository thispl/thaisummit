...
def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status,attendance_request,leave_type from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
...
def get_employee_details(group_by, company):
	...
	query = """select name, employee_name, designation, department, branch, company,
		holiday_list from `tabEmployee` where company = %s and employee_type = 'WC' """ % frappe.db.escape(company)
